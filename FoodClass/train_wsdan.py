...
def main():
    parser = OptionParser()
    parser.add_option('-j', '--workers', dest='workers', default=32, type='int',
    ...
    parser.add_option('--lr', '--learning-rate', dest='lr', default=4e-3, type='float',
                      help='learning rate (default: 1e-3)')
    parser.add_option('--sf', '--save-freq', dest='save_freq', default=1, type='int',
                      help='saving frequency of .ckpt models (default: 1)')
    parser.add_option('--sd', '--save-dir', dest='save_dir', default='./food_models_583_lr4e-3',
    ...
    # Load dataset
    ##################################
    trainPath = '/data1/pub/dataset/food_583_splited/train/'
    valPath = '/data1/pub/dataset/food_583_splited/val/'

    normalize = transforms.Normalize(mean=[0.6301, 0.5246, 0.3962], std=[0.2747, 0.2881, 0.3135])

    train_dataset = ImageFolder(trainPath, transforms.Compose([
    ...
    validate_dataset = ImageFolder(valPath, transforms.Compose([
        transforms.Resize((299,299)),
        transforms.ToTensor(),
        normalize,
        ]))
...
def train(**kwargs):
    ...
    theta_c = 0.5
    theta_d = 0.5
    crop_size = (200, 200)  # size of cropped images for 'See Better'
    ...
    for i, (X, y) in enumerate(data_loader):
        ...
        epoch_loss[0] = epoch_loss[0] + batch_loss.item()
        ...
        feature_center[y] =feature_center[y] + beta * (feature_matrix.detach() - feature_center[y])

        # metrics: top-1, top-3, top-5 error
        with torch.no_grad():
            epoch_acc[0] = epoch_acc[0] + accuracy(y_pred, y, topk=(1, 2, 5))
        ...
        epoch_loss[1] = epoch_loss[1] + batch_loss.item()
        ...
        # metrics: top-1, top-3, top-5 error
        with torch.no_grad():
            epoch_acc[1] = epoch_acc[1] + accuracy(y_pred, y, topk=(1, 2, 5))
        ...
        # loss
        batch_loss = loss(y_pred, y)
        epoch_loss[2] = epoch_loss[2] + batch_loss.item()
        ...
        # metrics: top-1, top-3, top-5 error
        with torch.no_grad():
            epoch_acc[2] = epoch_acc[2] + accuracy(y_pred, y, topk=(1, 2, 5))
...
def validate(**kwargs):
    ...
    # Default Parameters
    theta_c = 0.5
    crop_size = (150, 150)  # size of cropped images for 'See Better'
